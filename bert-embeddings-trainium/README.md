...
2. **vLLM-Neuron container lacks `torch_neuronx.nki_hop`.** Customer-written NKI kernels can't be dispatched from a vLLM-served model — the bridge isn't installed in the vllm-neuron private-beta image. NKI fused kernels work fine on the **native** path. Reported as a feature request.

...
"""Tiny wrapper to apply the Qwen3.6 registry patch and exec `vllm serve`.
...
- MODEL: path to local HF weights (default: /root/models/Qwen3.6-4B)
- TP: tensor parallel size (default: 4). Hard caps: 16 query heads on
  Qwen3.6-4B → max TP=16. KV heads = 4 → above TP=4, KV is replicated.
...
def main() -> int:
    ...
    model = os.environ.get("MODEL", "/root/models/Qwen3.6-4B")

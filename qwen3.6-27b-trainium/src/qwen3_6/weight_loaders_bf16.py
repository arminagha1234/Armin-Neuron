...
"""Weight loaders for Qwen3.6 BF16 checkpoint format — Phase 6.
...
HuggingFace checkpoint layout for Qwen3.6-27B
(`model.safetensors.index.json`):
  ...
  Per layer L (0..31):
    ...
    DeltaNet linear-attention layers (L in [0,1,2, 4,5,6, 8,9,10, ...]):
      ...
      model.layers.{L}.linear_attn.conv1d.bias         [conv_dim]            (we ignore — Qwen3.6 uses bias=False)
...
- We don't load `conv1d.bias` (Qwen3.6 conv has no bias).
...
def build_weight_mappings(config: Qwen3_6Config) -> dict[str, list[str]]:
    ...
    # HF prefix: Qwen3.6-27B is a multimodal HF wrapper, so all text-decoder
    ...
    # LM head — Qwen3.6-27B has tie_word_embeddings=False, so lm_head
    # is its own checkpoint key at the TOP level (NOT under language_model.).
    # This is the key behavioral diff from 4B (which was tied).
    ...
    for L, lt in enumerate(config.layer_types):
        ...
        if lt == "full_attention":
            # GQA: fuse Q + K + V into one tensor. For Qwen3.6 the on-disk
            # q_proj.weight has shape (2*Q*head_dim, hidden) when
            # attn_output_gate=True — first half is real Q, second half is
            # the attention output gate. Our custom _spliced_q_kv_loader
            # picks the right rows. The same q_proj.weight is also referenced
            # by attn_gate_weight (which slices the second half).
            ...
            # Attention output gate (Qwen3.6-specific; spliced into q_proj's
            # second half). Mapped when config.attn_output_gate is True.
...
def expected_unmapped_keys(config: Qwen3_6Config) -> set[str]:
    ...
    For Qwen3.6-27B specifically:
      - mtp.* — multi-token prediction head (we don't run MTP yet)
      - model.visual.* — vision tower (text-only serve)
      - linear_attn.conv1d.bias — present in checkpoint but bias=False
        in code (Qwen team ships zero bias defensively).
      - lm_head.weight is NOT in the skip set for 27B because we DO map
        it explicitly (tie_word_embeddings=False).
    """
    skip: set[str] = set()
    # conv1d biases (Qwen3.6 sets bias=False but the safetensors still
    # ship them as zeros for round-trip safety)
    for L, lt in enumerate(config.layer_types):
        if lt == "linear_attention":
            skip.add(f"model.language_model.layers.{L}.linear_attn.conv1d.bias")
    return skip


def expected_unmapped_prefixes() -> tuple[str, ...]:
    """Whole subtrees of the checkpoint we skip wholesale.

    Returned as a tuple of prefix strings. Any HF safetensors key whose
    name starts with one of these prefixes is silently ignored.

    Used for:
      - mtp.* — multi-token prediction head, separate runtime
      - model.visual.* / model.language_model.visual.* — vision tower,
        not used for text-only serve
    """
    return (
        "mtp.",
        "model.visual.",

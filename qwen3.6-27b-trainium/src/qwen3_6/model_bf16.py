...
"""Qwen3.6 hybrid (DeltaNet + GQA) model — BF16 implementation.
...
# ============================================================================
# Custom weight loaders for Qwen3.6's spliced q_proj layout
# ----------------------------------------------------------------------------
# Qwen3.6's HF safetensors store q_proj as [2*Q*head_dim, hidden] when
# attn_output_gate=True: rows 0..Q*head_dim are the real Q projection,
# rows Q*head_dim..2*Q*head_dim are the attention output gate. Concretely
# for 27B: q_proj.weight has shape (12288, 5120) instead of the usual
# (6144, 5120).
#
# We can't use the stock fused_qkv_weight_loader because it asserts
# len(Q slice shape) == 2 and assumes the slice's leading dim equals
# n_q*head_dim. So we write a thin wrapper that builds a custom
...
def _spliced_q_kv_loader(q_size_full, kv_size_full, num_shards, num_kv_replicas,
    ...
    Qwen3.6 q_proj on disk has shape (num_heads * head_dim * 2, hidden),
    laid out per-head INTERLEAVED:
...
class Qwen3_6RMSNorm(nn.Module):
    ...
    IMPORTANT: Qwen3.6 (like Gemma) stores RMSNorm weights centered around
    ...
    Using plain `weight * x` makes every norm in the network ~1.0 too
    small, which uniformly inflates/garbles activations and produces
    degenerate output. This applies to ALL norms: input_layernorm,
    post_attention_layernorm, q_norm, k_norm, and the final model norm.
    """
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # (1 + weight) convention — the critical Qwen3.6 detail.
...
#   - `attn_output_gate` flag (Qwen3.6 has a sigmoid gate on attn output)
# ============================================================================
...
class Qwen3_6GQAAttention(nn.Module):
    ...
    def __init__(self, config: Qwen3_6Config, layer_idx: int) -> None:
        ...
        # Qwen3.6-27B's HF config flags attn_output_gate=True AND the
        # safetensors DO ship the gate, but it's spliced into q_proj's
        # second half rather than as a separate gate_proj.weight. So
        # the on-disk q_proj.weight has shape [2*Q*head_dim, hidden]:
        # the first Q*head_dim rows are the real Q projection, the
        # second Q*head_dim rows are the attention output gate.
        # We honour `config.attn_output_gate` here. The custom loader
        # in _setup_weight_loaders splits q_proj's two halves into our
        # qkv_proj_weight (Q half + K + V) and attn_gate_weight (gate
        # half).
    ...
    def _setup_weight_loaders(self) -> None:
        if self.attn_output_gate or getattr(self, "_use_spliced_q_loader", False):
    ...
    def forward_decode(
        ...
        on Qwen3.6's head_dim=256. So we follow the prefill pattern:
...
class Qwen3_6DeltaNetAttention(nn.Module):
    ...
    Wraps `qwen3_6.nki_kernels.deltanet_fused_chunked_fwd` (PR #152).
    ...
    def forward(
        ...
        both paths plumbed. Weights still replicated across TP
        ranks (head sharding is straightforward since the kernel is
        per-(b,h), but pushes to Phase 5.5 to keep this commit focused).
        """
    ...
    def _forward_prefill(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # 13. Stash recurrent state for decode path (Phase 5 will read this)
        ...
        # Buffer-dependency trick (PR #152 pattern): forces the alias.
...
class Qwen3_6DecoderLayer(nn.Module):
    ...
    For Qwen3.6: layer_types[layer_idx] decides whether self_attn is
    a full-attention GQA layer or a linear-attention DeltaNet layer.
    """

    def __init__(self, config: Qwen3_6Config, batch_size: int, layer_idx: int) -> None:
        super().__init__()
        self.layer_idx = layer_idx
...
class Qwen3_6Model(nn.Module):
    """Qwen3.6 transformer backbone."""

    def __init__(self, config: Qwen3_6Config, batch_size: int) -> None:
        super().__init__()
        self.config = config
        ...
        self.layers = nn.ModuleList(
            Qwen3_6DecoderLayer(config, batch_size, i)
...
class Qwen3_6ForConditionalGeneration(nn.Module):
    ...
    def __init__(self, config: Qwen3_6Config, batch_size: int = 1) -> None:
        super().__init__()
        self.config = config
        self.model = Qwen3_6Model(config, batch_size=batch_size)

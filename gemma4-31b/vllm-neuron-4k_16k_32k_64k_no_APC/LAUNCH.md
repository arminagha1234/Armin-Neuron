...
End state: Gemma 4-31B served on Trainium via vLLM-Neuron in the **best-TTFT config**
(single-shot ≤16k / segmented >16k, **no APC**, bf16), with honest **cold** TTFT/TPOT/E2E
numbers for 4k/8k/16k/32k/64k. Every request uses a unique random prompt (no cache reuse).
...
### 10b. [CONTAINER] Apply the best-TTFT model patches (recommended)
The winning config fuses QKV+QK-norm+RoPE and the o_proj matmul into NKI kernels, and routes
>16k segmented prefill through the NKI flash kernel. These are runtime edits to the installed
`vllm_neuron` model file (idempotent, self-backing-up). See `patches/APPLY_AND_TEST.md` for detail.
```bash
python3 patches/patch_qkv_proj.py       # fused QKV+QK-norm+RoPE (TP>=8)
python3 patches/patch_oproj.py          # o_proj NKI kernel (prefill)
python3 patches/patch_segmented_nki.py  # NKI flash for >16k segmented prefill
# optional CPU correctness gate (cosine vs torch): GEMMA4_TEST_USE_KERNEL=0 python3 patches/test_correctness.py
```
...
```
Ballpark **cold** TTFT (trn2.48xlarge, TP=32, concurrency 1, no APC, bf16): ~0.22 s @4k,
~0.39 s @8k, ~0.75 s @16k (single-shot), ~3.06 s @32k, ~6.09 s @64k (segmented). Full tables
are in `RESULTS.md`. These are cold/unique-prompt numbers — no prefix-cache reuse.
...
- **Server "hangs" for 10-15 min on first launch** — that's the cold NEFF compile; it's cached
  afterward. Watch it: `sudo docker exec vllm_neuron tail -f /root/Armin-Neuron/gemma4-31b/vllm-neuron-4k_16k_32k_64k_no_APC/results_*/serve_len*.log`

#!/usr/bin/env python3
"""Distribution-aware TTFT bench for GLM 5.1 on vLLM-Neuron — mirrors the
Gemma 4 31B harness (`vllm-neuron-gemma4-31b/bench_distribution.py`) so
the weighted-average TTFT is directly comparable across models.

Default distribution (same payload mix used for the Gemma 4 31B run):
  <=0.5K  24.8%   <=1K  53.1%   <=2K  9.5%   <=4K  12.7%

NOTE: baseline pass — many optimizations remain (FP8-on-device, EP,
multi-bucket compile, NKI router). See BENCHMARK.md.

Run inside the container after the server is up:
    python3 bench_distribution.py --model <model-id-or-path>
"""
...
def main():
    ...
    ap.add_argument("--model", default="/root/models/glm5.1")
    ...
    print(f"Distribution-aware TTFT bench (GLM 5.1)  model={args.model}")

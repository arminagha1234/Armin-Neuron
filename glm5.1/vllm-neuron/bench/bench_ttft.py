#!/usr/bin/env python3
"""TTFT benchmark for GLM 5.1 on vLLM-Neuron — mirrors the Gemma 4 31B
harness (`vllm-neuron-gemma4-31b/bench_ttft.py`) byte-for-byte except the
default model id, so numbers are directly comparable across models.

Streaming completions, unique random-token prompts (no prefix-cache
inflation), median of N runs per sequence length. JSON out.

NOTE: this is a baseline pass — see `BENCHMARK.md` for the optimization
roadmap. We can do much better with FP8-on-device weights + EP +
multi-bucket compile.

Run INSIDE the container after the server is up:
    python3 bench_ttft.py --model <model-id-or-path> \\
        --tag glm5.1-tp32 --out /work/results/glm5_ttft.json
"""
...
def main():
    ...
    ap.add_argument("--model", default="zai-org/GLM-5.1")
    ap.add_argument("--base-url", default="http://localhost:8000")
    ap.add_argument("--seq-lens", default="256,512,1024,2048,4096,8192")
    ap.add_argument("--runs", type=int, default=5)
    ap.add_argument("--tag", default="pathA")
    ap.add_argument("--out", default="/work/results/glm5_ttft.json")
    args = ap.parse_args()

    seq_lens = [int(x) for x in args.seq_lens.split(",")]
    print(f"GLM 5.1 TTFT bench [{args.tag}]  model={args.model}  seq_lens={seq_lens}")
    results = []
    for sl in seq_lens:
        print(f"--- seq_len={sl} ---")
    ...
    print("\n" + "=" * 48)
    print(f"GLM 5.1 TTFT [{args.tag}] vs {TARGET_MS:.0f} ms")

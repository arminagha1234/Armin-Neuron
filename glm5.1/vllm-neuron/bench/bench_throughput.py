#!/usr/bin/env python3
"""GLM 5.1 throughput sweep — max tokens/min via concurrent clients.
Mirrors the Gemma 4 31B `bench_throughput.py` harness so numbers compare
directly across models.

Reports aggregate output tokens/s and tokens/min at increasing concurrency,
so we can find the max sustained throughput.

NOTE: baseline run — heavy MoE routing overhead is the limiter; future
work (EP, FP8 on-device) lifts this ceiling. See BENCHMARK.md.

Run inside the container with the server already up on :8000.
    python3 bench_throughput.py --model <model-id-or-path> \\

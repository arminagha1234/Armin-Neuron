#!/usr/bin/env python3
"""Generate the Indeed capacity-request progress charts.
...
def chart_gap():
    ...
    ax.set_title("How far each model is from Indeed's RPS target")
    ax.grid(axis="x", color=GRID, lw=0.8)
    ax.set_axisbelow(True)

    handles = [
        plt.Line2D([], [], marker="|", color=INK, lw=0, markersize=14,
                   markeredgewidth=2.6, label="Indeed target"),
...
def chart_pct():
    ...
    ax.annotate("Indeed target = 100%", (len(names) - 0.42, 100),
    ...
    ax.set_yscale("log")
    ax.set_ylim(3, 1400)
    ax.set_ylabel("% of Indeed's RPS target achieved on one box (log)")
...
# ============================================================ chart 5 =======
# Boxes required -- the capacity answer Indeed actually cares about.
def chart_boxes():
    ...
    ax.set_title("Capacity required per model at Indeed's target RPS")
    ax.grid(axis="x", color=GRID, lw=0.8)
    ax.set_axisbelow(True)
...
def chart_instances():
    ...
    ax.set_title("Instances required at Indeed's asks: trn2.3xlarge vs trn2.48xlarge")
...
def chart_answer_card():
    ...
    ax.set_title("Which Trn2 instance does each Indeed ask need?  (5 asks across 4 models)",

...
def main(model_path: str = "/root/models/Qwen3.5-4B") -> int:
    print("=" * 70)
    print("Qwen3.5-4B Path B Phase 7 smoke test")
    print("=" * 70)
    ...
    from qwen3_5.weight_loaders_bf16 import (
    ...
    # Every HF key must be either mapped or explicitly skipped
    skip = expected_unmapped_keys(cfg)
    leftover = hf_keys - referenced - skip
    if leftover:
        print()
        print(f"  FAIL: {len(leftover)} HF keys are neither mapped nor explicitly skipped:")
        for k in sorted(leftover)[:10]:
            print(f"    - {k}")
        return 3

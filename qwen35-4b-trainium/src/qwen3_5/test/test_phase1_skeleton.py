...
PYTHONPATH:
    ...
    python -m qwen3_5.test.test_phase1_skeleton

Validates:
1. register() patches vllm_neuron.model.registry without crashing
2. Qwen3_5Config builds from the real Qwen/Qwen3.5-4B config.json
...
# When run as `python -m qwen3_5.test.test_phase1_skeleton`, sys.path is
...
def main(model_path: str = "/root/models/Qwen3.5-4B") -> int:
    from qwen3_5 import Qwen3_5Config, Qwen3_5ForConditionalGeneration
    ...
    print("[3/4] Qwen3_5Config.from_configs ... ", end="", flush=True)
    ...
    if not os.path.isfile(cfg_path):
        ...
        with open(cfg_path) as f:
            hf_cfg = json.load(f)
        cfg = Qwen3_5Config.from_configs(hf_cfg, neuron_config=None)
        assert cfg.num_hidden_layers == 32, cfg
        assert cfg.num_full_attention_layers == 8, cfg.num_full_attention_layers
        assert cfg.num_linear_attention_layers == 24, cfg.num_linear_attention_layers
...
if __name__ == "__main__":
    path = sys.argv[1] if len(sys.argv) > 1 else "/root/models/Qwen3.5-4B"

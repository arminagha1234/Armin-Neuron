...
Reference workload: an external image-to-image editing zoom-LoRA on
top of FLUX.2-klein 4B base. The LoRA is a transformer-only adapter
    ...
    def forward(self, *args, **kwargs):
        ...
        for key in (
            ...
            if v is None or not torch.is_tensor(v):
                continue
            if v.device != target:
                # CPU → Neuron: make contiguous BEFORE the device move
                kwargs[key] = v.contiguous().to(device=target)
            else:
                # Already on Neuron: if non-contiguous, round-trip via
                # CPU to rebuild contiguity (Neuron lazy backend can't
                # make a device-resident view contiguous in-place).
                if not v.is_contiguous():
                    kwargs[key] = v.to(device="cpu").contiguous().to(device=target)

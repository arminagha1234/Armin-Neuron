...
## Headline — 4-step distilled config (current production, MEASURED)

| Path | Warm avg | Std | $/image | vs p5.4xl ($0.00059) | Latency vs measured H100 4-step (0.49 s) |
|---|---:|---:|---:|---:|---:|
| **H100 1× p5.4xl Capacity Blocks (MEASURED)** | **0.49 s** | 76.3 | **$0.00059** | baseline | 1× (reference) |
| trn2.3xl single-core (single-stream entry) | 4.19 s | 18.16 | $0.00260 | ~4.4× more expensive | ~8.5× slower |
| CPU VAE channels_last (prior shipped, 32-core saturated) | 5.92 s | 18.15 | ~$0.00110 | ~1.9× more expensive | ~12× slower |
| **VAE on Neuron + PAVE fixes + mixed flags (NEW DEFAULT, 32-core saturated)** | **4.19 s** ✅ | **18.16** | **~$0.00078** | **~1.3× more expensive** | **~8.5× slower** |

**Honest read of the table:**

- **H100 measured at 0.49 s / $0.00059** is faster AND cheaper than
  the 32-core-saturated Trainium row. The earlier "Trainium 25%
  cheaper" headline came from an extrapolation (`6.1 s × 4 / 28 ≈
  0.87 s`) that overestimated H100 4-step latency. The measured
  4-step number is 1.8× faster than the extrapolation, which flips
  the cost comparison.
- The −29% Trainium-internal speedup (5.92 s → 4.19 s) from the
  VAE-on-Neuron + PAVE-fixes + mixed-flag work is **still real and
  unchanged** — that's the right Trainium-vs-Trainium comparison.
- Trainium remains a sensible choice for: (a) lower hourly entry
  pricing — trn2.3xl at $2.23/hr is the cheapest hourly here; (b)
  inventory / availability constraints when H100 capacity is hard to
  get; (c) workloads that benefit from TP/CP at sizes Trainium fits
  natively. It is not, today, a $/image win at 4-step on this
  benchmark.

**Win: −29% Trainium-internal speedup, lossless quality.** See
...
(The headline table above costs H100 at the **p5.4xl Capacity Blocks
rate of $4.326/hr** for the apples-to-apples comparison: 0.49 s × $4.326/3600
= $0.00059/img. The "1/8 of p5.48xl box" column uses the on-demand p5.48xl
rate divided by 8 GPUs and gives a slightly cheaper number; both are
defensible depending on the SKU the customer actually buys.)

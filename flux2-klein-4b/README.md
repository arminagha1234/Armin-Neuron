...
| Path | Best for | 1024² × 28 steps | Per-step | $/image |
|---|---|---:|---:|---:|
| [`native-pytorch/`](native-pytorch/) | **Lowest-latency standalone** | **65.9 s** | **2.35 s** | **$0.041** |
| [`vllm-omni/`](vllm-omni/) | Multi-modal serving stack | extrapolated >800 s | ~150 s/step | extrapolated $4.7 |

(Cost based on trn2.3xlarge $2.23/hr for native path; trn2.48xlarge
$21.50/hr for vllm-omni.)

The native PyTorch path is ~15× faster per-step because it skips the
omni engine layer. **Use it for production serving when FLUX.2-klein
is the only model on the box.** The vllm-omni path is the right
answer when FLUX.2-klein is hosted alongside other modalities (LLM +
video + audio) inside a single omni service.
...
```
   ...
   "Just FLUX.2-klein, fastest"                "FLUX inside multi-modal omni"
       ...
       65.9s @ 1024²/28 steps                         needs omni engine
       $0.041/image                                   shared scheduler/KV
       26% cheaper than H100                          + other modalities
```

## Highlights

### Native PyTorch path
- **65.9 s** for 28-step 1024×1024 with `torch.compile(backend="neuron")`
- **26% cheaper than H100** at the same workload ($0.041 vs $0.055/image)

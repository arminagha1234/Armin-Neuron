...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # NEURON PORT: use SDPA (no Flash Attention on Trainium)
        # Use is_causal=True (supported in Beta 3 forward-only SDPA)
...
# NEURON PORT: remove @torch.compile from optimizer fused steps
# (they run on Neuron tensors; Inductor doesn't support device "neuron")
...
# NEURON PORT: remove @torch.compile (Inductor doesn't support device "neuron")
...
torch.manual_seed(42)  # NEURON PORT: no torch.cuda.manual_seed
...
model.init_weights()
model = model.to(dtype=torch.bfloat16)  # NEURON PORT: bf16 natively
...
while True:
    ...
    mfu = 100 * num_flops_per_token * TOTAL_BATCH_SIZE / dt / TRN2_BF16_PEAK_FLOPS
...
steady_state_mfu = 100 * num_flops_per_token * TOTAL_BATCH_SIZE * (step - 10) / total_training_time / TRN2_BF16_PEAK_FLOPS if total_training_time > 0 else 0
